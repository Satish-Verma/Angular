          ...
          post: function postlink($scope, $elm, $attrs, controllers) {
            ...
            gridUtil.on.mousewheel($elm, function (event) {
              ...
              if (event.deltaY !== 0) {
                ...
                else if (scrollYPercentage > 1) { /*sheel scrollYPercentage = 1;*/ }
  ...
  angular.module('ui.grid').controller('uiGridController', ['$scope', '$element', '$attrs', 'gridUtil', '$q', 'uiGridConstants',
    ...
    function ($scope, $elm, $attrs, gridUtil, $q, uiGridConstants,
      ...
      function dataWatchFunction(newData) {
        ...
        //sheel
        if(self.grid.selection != undefined)
        {
    ...
    function ($q, $templateCache, uiGridSelectionConstants, gridUtil) {
      ...
      var service = {
          ...
          var publicApi = {
            ...
            methods: {
              selection: {
                ...
                //sheel
                setSelectAllState: function (val) {
                    return grid.selection.selectAll = val;
                  }
        ...
        toggleRowSelection: function (grid, row, evt, multiSelect, noUnselect) {
          ...
          //sheel
          grid.selection.selectAll =(grid.api.selection.getSelectedRows().length == grid.rows.length);
        },
        /**
         * @ngdoc function
         * @name shiftSelect
      ...
      return {
        ...
        link: function($scope, $elm, $attrs, uiGridCtrl) {
          ...
          function selectButtonClick(row, evt) {
            evt.stopPropagation();
            //sheel
            if(row.enableSelection !== undefined  && !row.enableSelection)
            	return false;
            if (evt.shiftKey) {
  ...
  function ($templateCache, uiGridTreeBaseService) {
    return {
      ...
      link: function($scope, $elm, $attrs, uiGridCtrl) {
        ...
        //sheel
        self.api.grid.api.treeButtonClick = $scope.treeButtonClick;
      }
    };
  }]);
...
angular.module('ui.grid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui-grid/ui-grid-menu-button',
		  //sheel
    "<div class=\"ui-grid-menu-button\" ng-click=\"toggleMenu()\"><div class=\"ui-grid-icon-container\"><span class=\"lnr lnr-menu icon-size\"></span></div><div ui-grid-menu menu-items=\"menuItems\"></div></div>"
  );
  ...
  $templateCache.put('ui-grid/ui-grid-row',
    "<div ng-mouseover=\"row.isHover=true;\" ng-mouseout=\"row.isHover=false;\" ng-repeat=\"(colRenderIndex, col) in colContainer.renderedColumns track by col.uid\" class=\"ui-grid-cell\" ng-class=\"{ 'ui-grid-row-header-cell': col.isRowHeader, 'ui-grid-cell-hover':row.isHover }\" ui-grid-cell></div>"
  );
  ...
  $templateCache.put('ui-grid/uiGridHeaderCell',
    "<div ng-class=\"{ 'sortable': sortable }\"><!-- <div class=\"ui-grid-vertical-bar\">&nbsp;</div> --><div class=\"ui-grid-cell-contents\" col-index=\"renderIndex\" title=\"TOOLTIP\"><span>{{ col.displayName CUSTOM_FILTERS }}</span> <span ui-grid-visible=\"col.sort.direction\" class=\"pull-right\" ng-class=\"{ 'ui-grid-icon-up-dir': col.sort.direction == asc, 'ui-grid-icon-down-dir': col.sort.direction == desc, 'ui-grid-icon-blank': !col.sort.direction }\">&nbsp;</span></div><div class=\"ui-grid-column-menu-button\" ng-if=\"grid.options.enableColumnMenus && !col.isRowHeader  && col.colDef.enableColumnMenu !== false\" ng-click=\"toggleMenu($event)\" ng-class=\"{'ui-grid-column-menu-button-last-col': isLastCol}\"><i class=\"ui-grid-icon-angle-down\">&nbsp;</i></div><div ui-grid-filter></div></div>"
  );
  ...
  $templateCache.put('ui-grid/uiGridViewport',
    "<div class=\"ui-grid-viewport\" ng-style=\"colContainer.getViewportStyle()\"><div class=\"ui-grid-canvas\"><div ng-repeat=\"(rowRenderIndex, row) in rowContainer.renderedRows track by $index\" class=\"ui-grid-row\" ng-style=\"Viewport.rowStyle(rowRenderIndex)\"><div ui-grid-row=\"row\" row-render-index=\"rowRenderIndex\"></div></div></div></div>"
		  //sheel
	//"<div class=\"ui-grid-viewport\" ng-style=\"colContainer.getViewportStyle()\" ng-ui-grid-scroll><div id=\"#scroller\"><div class=\"ui-grid-canvas\"><div ng-repeat=\"(rowRenderIndex, row) in rowContainer.renderedRows track by $index\" class=\"ui-grid-row\" ng-style=\"Viewport.rowStyle(rowRenderIndex)\"><div ui-grid-row=\"row\" row-render-index=\"rowRenderIndex\"></div></div></div></div></div>"
	 // "<div class=\"ui-grid-viewport\" ng-style=\"colContainer.getViewportStyle()\"><div style=\"width:100%;height:100%;overflow:hidden;\" ng-ui-grid-scroll><div id=\"#scroller\"><div class=\"ui-grid-canvas\"><div ng-repeat=\"(rowRenderIndex, row) in rowContainer.renderedRows track by $index\" class=\"ui-grid-row\" ng-style=\"Viewport.rowStyle(rowRenderIndex)\"><div ui-grid-row=\"row\" row-render-index=\"rowRenderIndex\"></div></div></div></div></div></div>"
  );
  ...
  $templateCache.put('ui-grid/expandableRow',
    "<div ui-grid-expandable-row ng-if=\"expandableRow.shouldRenderExpand()\" class=\"expandableRow\" style=\"float:left; margin-top: 0px; margin-bottom: 1px\" ng-style=\"{width: (grid.renderContainers.body.getCanvasWidth()) + 'px'\n" +
  ...
  $templateCache.put('ui-grid/selectionRowHeaderButtons',
    "<div class=\"ui-grid-selection-row-header-buttons\" ng-class=\"{'ui-grid-row-selected': row.isSelected}\"><div ng-click=\"selectButtonClick(row, $event)\" class=\"ui-grid-rms-icon-ok\" ng-class=\"{'ui-grid-rms-icon-ok-hover':row.isHover,'ui-grid-rms-icon-ok-selected':row.isSelected}\"></div></div>"
  );


  $templateCache.put('ui-grid/selectionSelectAllButtons',
		  //sheel
    "<div class=\"ui-grid-selection-row-header-buttons\" ng-class=\"{'ui-grid-all-selected': grid.selection.selectAll}\"><div ng-click=\"grid.rows.length > 0 && headerButtonClick($event)\" class=\"ui-grid-rms-icon-ok\" ng-class=\"{'ui-grid-rms-icon-ok-hover':row.isHover,'ui-grid-rms-icon-ok-selected':grid.selection.selectAll}\"></div></div>"
  );
